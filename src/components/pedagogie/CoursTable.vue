<!-- src/components/pedagogie/CoursTable.vue -->
<template>
  <!-- Vue “cards” mobile -->
  <div class="block md:hidden divide-y divide-slate-100 dark:divide-slate-800">
    ...
    >
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          <div class="font-semibold">{{ r.intitule }}</div>
      ...
      <div class="mt-2 grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-300">
        <div><span class="opacity-60">Classe:</span> {{ r._classeLabel }}</div>
        <div><span class="opacity-60">Prof:</span> {{ r._profLabel }}</div>
        <div><span class="opacity-60">Période:</span> {{ r._periodeLabel }}</div>
        <div><span class="opacity-60">Volume:</span> {{ r.volume ?? 0 }}</div>
      </div>
    </div>
  </div>
  ...
  <table class="hidden min-w-full text-left text-sm md:table">
    ...
    <tbody>
      ...
      >
        ...
        <td class="px-4 py-3">{{ r.intitule }}</td>
        <td class="px-4 py-3">{{ r.matiere || '—' }}</td>
        <td class="px-4 py-3">{{ r._classeLabel }}</td>
        <td class="px-4 py-3">{{ r._profLabel }}</td>
        <td class="px-4 py-3">{{ r._periodeLabel }}</td>
        <td class="px-4 py-3">{{ r.volume ?? 0 }}</td>
        <td class="px-4 py-3">
          <div class="flex justify-end gap-2">
            <RouterLink
              :to="`/cours/detail?id=${r.id}`"
