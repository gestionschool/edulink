<!-- src/pages/pedagogie/InterrosList.vue -->
<template>
  <div class="space-y-6">
    <!-- Header -->
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400">
          Gestion des interros par cours / classe / période
        </p>
      </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div v-else-if="rows.length === 0" class="p-6 text-sm text-slate-500 dark:text-slate-400">
        Aucune interro trouvée.
      </div>

      <div v-else class="overflow-x-auto">
        <InterrosTable :items="rows" :total="total" @remove="onRemove" />
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  await Promise.all([
    ...
    interros.fetch({ _sort: 'date', _order: 'desc' }),
  ])
})
...
const typesOptions = computed(() => {
  const vals = new Set((interros.items||[]).map(r => r.type).filter(Boolean))
...
const rows = computed(() => {
  ...
  return (interros.items||[])
    ...
    .filter(r => {
      ...
      if (!term) return true
      const hay = `${r.titre} ${r.type} ${r._coursLabel} ${r._matiere} ${r._classeLabel} ${r._profLabel} ${r._periodeLabel}`.toLowerCase()
      return hay.includes(term)
    })
})

const total   = computed(() => interros.items?.length || 0)
const loading = computed(() => interros.loading || cours.loading || classes.loading || periodes.loading || teachers.loading)
const error   = computed(() => interros.error   || cours.error   || classes.error   || periodes.error   || teachers.error)

async function onRemove(id){
  if (!confirm('Supprimer cette interro ?')) return
  await interros.removeOne(id)
  await interros.fetch({ _sort: 'date', _order: 'desc' })
}
</script>
